...
import { Partner, Subscription, Liquidation, PayableItem, PayoutRecord, CommercialPlan, Company } from '../types';
...
interface LiquidationBuilderProps {
  ...
  companies: Company[]; // Added missing prop
...
const LiquidationBuilder: React.FC<LiquidationBuilderProps> = ({ 
  ...
  liquidations,
  commercialPlans, 
  companies, // Destructured missing prop
  ...
  const allPayableItems = useMemo(() => {
    // Fixed: Added companies as the 6th argument
    return generateAccountStatus(null, subscriptions, liquidations, partners, commercialPlans, companies);
  }, [subscriptions, liquidations, partners, commercialPlans, companies]);
  ...
  const downloadCSV = (items: Liquidation[]) => {
    ...
    const rows = items.map(item => [
        item.ID_Liquidacion,
        item.ID_Partner,
        `"${item.Cliente || ''}"`, // Handled missing Cliente with fallback
        item.Mes_Pagado,
      ...
      <div className="space-y-4">
        {partners.map(partner => {
            ...
            const partnerTotalMRR = subscriptions
                .filter(s => {
                    const company = companies.find(c => c.id === s.id_cliente);
                    return company?.id_partner === partner.ID_Partner && s.Estado === 'Activo';
                })
                    ...
                    {isPExpanded && (
                        <div className="divide-y divide-slate-100 bg-white">
                            ...
                            {clientNames.map(clientName => {
                                ...
                                const currentMRR = items[0]?.Importe || 0; // Fallback to item amount if subscription lookup fails
